...
public class MenuDao {
	...
	/*
	 * public static MenuViewDataset getAllMenuData(MrBean user, Connection conn)
	 * throws SQLException {
	 * 
	 * ArrayList<MenuViewData> ret = new ArrayList<MenuViewData>();
	 * 
	 * ArrayList<DBRecord> dbrs = DBMgr.getDBRecords(defineView(), "",
	 * " ORDER BY syskey", conn);
	 * 
	 * for (int i = 0; i < dbrs.size(); i++) {
	 * ret.add(getDBViewRecord(dbrs.get(i))); }
	 * 
	 * MenuViewDataset dataSet = new MenuViewDataset(); MenuViewData[] dataarry =
	 * new MenuViewData[ret.size()]; dataarry = ret.toArray(dataarry);
